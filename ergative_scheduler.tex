...
\node[symbox, minimum width=4.5cm, minimum height=1.5cm, below=0.6cm of title] (sym) {
  \normalfont\textbf{Before Interrupt} \\
  \normalfont\tiny $P_{\pm}^{\text{sym}}$ -- all processes undifferentiated \\
  \normalfont\tiny \textcolor{orange!70!black}{is\_symmetric() = true} \\
  \normalfont\tiny \textcolor{red!70!black}{schedule\_next() = None}
};
...
\node[rectangle, draw=green!60!black, thick, dashed, rounded corners, fill=green!3,
  ...
  \normalfont\textbf{After Interrupt} \\
  \normalfont\tiny $P_{\text{asym}}$ -- ergative model activates \\
  \normalfont\tiny \textcolor{green!60!black}{is\_symmetric() = false} \\
  \normalfont\tiny \textcolor{green!60!black}{schedule\_next() = highest priority}
};
...
% Absolutive process (no targets)
\node[abs, below=0.4cm of erg_desc] (abs1) {\textbf{Absolut ive}\\targets $= \emptyset$};
...
% ============================================================
% RIGHT: Tier Priority + K\_trap Gate
% ============================================================
\node[font=\bfseries\small, text=purple!80!black, right=2.5cm of asym.east, anchor=west] (ttitle) {Tier Priority + K$_{\text{trap}}$ Gate};
...
\node[rectangle, draw=red!70!black, thick, rounded corners, fill=red!8, minimum width=3.5cm, minimum height=0.6cm,
  ...
  \textbf{K$_{\text{trap}}$ Gate}: if $K = K_{\text{trap}} \to C = 0$, cannot be scheduled
};
...
% Letters per tier
\node[font=\tiny, left=0.15cm of tinf.west, anchor=east, text=gray!60!black] (l_inf) {vav, mem, shin};
\node[font=\tiny, left=0.15cm of t2.west, anchor=east, text=gray!60!black] (l2) {aleph, hei, ayin, kuf, tav};
\node[font=\tiny, left=0.15cm of t2d.west, anchor=east, text=gray!60!black] (l2d) {lamed};
\node[font=\tiny, left=0.15cm of t1.west, anchor=east, text=gray!60!black] (l1) {lamed (alt)};
\node[font=\tiny, left=0.15cm of t0.west, anchor=east, text=gray!60!black] (l0) {12 letters};
...
\node[rectangle, draw=yellow!80!black, thick, rounded corners, fill=yellow!5, minimum width=8cm, minimum height=1.5cm,
  ...
  \normalfont\textbf{ProcessControlBlock} \{ \\
  \normalfont\quad id: \texttt{u64} \quad obj: \texttt{KernelObject} \quad role: \texttt{GrammaticalRole} \\
  \normalfont\quad priority: \texttt{u8} \quad stack\_pointer: \texttt{u64} \quad targets: \texttt{Vec<u64>} \\
  \normalfont\}
};

% Connections from PCB to roles
...
% Connection from K_trap to tier ladder
