...
% ============================================================
\node[phi_gate] (phig) {\textbf{$\Phi$-Gate}\\[0.2cm]
  depth 0--5:\\$\Phi \ge \Phi_c$\\[0.15cm]
  depth 6--10:\\$\Phi \ge \Phi_{\text{sub}}$\\[0.3cm]
...
\draw[dashed, thick, orange!60!black] ($(phig.south west)+(-0.1,0)$) -- ($(phig.south east)+(0.3,0)$)
  node[midway, above, font=\tiny\bfseries, text=orange!70!black] {$\Phi_c$ boundary};
...
\node[font=\tiny, left=0.1cm of phig.north west, anchor=north east, text=red!70!black, align=right] (phi_ann1) {
  $\Phi_c$ enforced\\$\Phi_c$ enforced\\$\Phi_c$ enforced\\$\Phi_c$ enforced\\$\Phi_c$ enforced\\$\Phi_c$ enforced
};
