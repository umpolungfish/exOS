...
\begin{tikzpicture}[
  ...
  \node[fill=blue!10] (s1) {$\Phi_{\pm}$ (perfect symmetry)\\Special Frobenius $\mu \circ \delta = \text{id}$};
  ...
  % Title
  \node[above=0.5cm of s1, font=\bfseries, align=center] {Boot Sequence: $\Phi_{\pm} \to \Phi_{\text{asym}}$\\Symmetry Breaking via First Interrupt};
\end{tikzpicture}
\end{document}
